...
1. **Direct confirmation/refutation**: Text directly agrees or disagrees with the claim
...
2. **Clarification/modification**: Text adds conditions, limitations, or nuances
   - Example: "Climate changes, but it's not the main problem"
   - Example: "Social networks are dangerous, but only with improper use"

3. **Contextual interpretation**: Text does not mention the proposition explicitly, but its content implies a position
...
4. **Relevance**: Determine whether text addresses the claim
   - If text is clearly unrelated to the proposition → Neutral position (off-topic)
...
5. **Intensity and emotionality**: Pay attention to the strength of position expression
   ...
   - Strong agreement: "Climate is catastrophically changing!"
...
7. **Modalities**: Words and phrases that qualify the relationship to the proposition:
   
   - **Agreement**: "indeed", "obviously", "of course", "true", "right", "I agree"
   - **Disagreement**: "but", "however", "on the contrary", "I disagree", "untrue", "wrong"
   - **Uncertainty**: "maybe", "perhaps", "seems", "appears", "probably"
   
   - **Opinion reference (without personal position)**: "the deputy stated that", "experts believe that", "in my opinion", "they say that"
     - Classification: Neutral (in the absence of explicit author position)
     - Exception: If the author adds an evaluation, it becomes a position
   
   - **Evaluative modality**: Words that carry implicit evaluation
     - Positive evaluation: "leading experts", "authorities", "recognised specialists", "correctly support"
     - Negative evaluation: "traitor", "liar", "charlatan", "incorrectly claim"
     - Classification: May be Positive/Negative depending on context
   
   Examples:
   - "Climate changes, but it's a natural process" → Disagreement (signal "but")
   - "Indeed, social networks are dangerous" → Agreement (signal "indeed")
   - "Perhaps digitalisation improves life" → Uncertainty (signal "perhaps")
   - "Experts believe that climate is changing" → Neutral (opinion reference without personal position)
   - "Leading climatologists confirm: climate is changing" → Positive (evaluative modality + agreement)
...
9. **Irony and sarcasm**: Be careful with irony and sarcasm
   ...
   Signals of irony (should be distinguished from simple expressiveness):
   - Exaggeration: "Social networks simply SAVE our lives!"
   - Contrast: "Yes, social networks are very useful" + negative context
